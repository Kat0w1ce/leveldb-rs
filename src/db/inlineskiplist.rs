...
impl Node {
    fn new<A: ArenaTrait>(key: Bytes, height: usize, arena: &A) -> *mut Self {
        let size =
...
struct InlineSkipListInner<A: ArenaTrait> {
...
unsafe impl<A: ArenaTrait + Send> Send for InlineSkipListInner<A> {}
unsafe impl<A: ArenaTrait + Sync> Sync for InlineSkipListInner<A> {}

impl<A: ArenaTrait> Drop for InlineSkipListInner<A> {
    fn drop(&mut self) {
...
mod tests {
    use super::*;
    use crate::util::arena::OffsetArena;
